...
def generate_recipe(ingredients):
    try:
        ...
        # Fetch recipes using Cosylab API
        
        url = f"https://cosylab.iiitd.edu.in/recipe-search/recipesAdvanced?pageSize=10"
        json = {
  "continent": "Asian",
  "region": "Middle Eastern",
  "subRegion": "",
  "recipeTitle": f"{ingredients}",
  "ingredientUsed": "fish",
  "ingredientNotUsed": "beef,chiicken,Lamb,meat,fish",
  "cookingProcess": "",
  "utensil": "",
  "energyMin": 0,
  "energyMax": 0,
  ...
  "proteinMax": 0
        ...
        if not recipes:
            return f"Error: No recipes found for ingredients {ingredients}."

        # Get the first recipe from the results
        recipe_data = recipes[0]
        dish = recipe_data.get("Recipe_title", "a dish")
        print(dish)

        # Generate recipe details using Gemini
        # gemini_prompt = f"Give me a simple recipe for {dish} using the ingredients: {', '.join(recipe_ingredients)}."
        gemini_prompt = f"Give me a simple recipe for {dish} in the format : dish name,ingredients,instructions. In not more than 300 words. Dont include any text in bold."
